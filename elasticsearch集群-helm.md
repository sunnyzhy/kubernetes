...
如果开启了 X-Pack，就需要配置 ```./charts/kibana/values.yaml```:
...
2. 如果开启了 X-Pack ，那么:
   - pod 启动时间会比较长
   - 需要修改 ```./charts/kibana/values.yaml``` 里的相关配置: ```elasticsearch.security.auth.enabled=true``` , ```elasticsearch.security.auth.kibanaPassword=root```
