...
***注: 横杠 ```-``` 表示去掉表达式输出结果前面和后面的空格，去掉前面空格可以这么写 ```{{- 模版表达式 }}```, 去掉后面空格 ```{{ 模版表达式 -}}```***
...
metadata:
  name: {{ .Release.Name }}-configmap
  labels:
    ## 引用 mychart.app 模版内容，并对输出结果缩进 4 个空格
