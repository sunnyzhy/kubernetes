...
```bash
# vim /usr/local/k8s/elasticsearch/elasticsearch/templates/service.yaml
```
...
kind: Service
metadata:
  name: {{ include "elasticsearch.service.name" . }}-service
...
```bash
# vim /usr/local/k8s/elasticsearch/elasticsearch/charts/kibana/templates/service.yaml
```
